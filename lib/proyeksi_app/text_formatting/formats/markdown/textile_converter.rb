...
module ProyeksiApp::TextFormatting::Formats
  module Markdown
    class TextileConverter
      ...
      def initialize
        @logger = ::ProyeksiApp::Logging::TeeLogger.new 'markdown-migration'
      ...
      def execute_pandoc_with_stdin!(textile, raise_on_timeout)
        ...
        if raise_on_timeout
          logger.error <<~TIMEOUT_WARN
            ...
            (PROYEKSIAPP_PANDOC_TIMEOUT_SECONDS, currently at #{pandoc.pandoc_timeout} seconds)
          TIMEOUT_WARN
      ...
      # ProyeksiApp support @ inside inline code marked with @ (such as "@git@github.com@"), but not pandoc.
