  ...
  module Logging
    class SentryLogger
      class << self
        ...
        # Sentry logging DSN
        def sentry_dsn
          @sentry_dsn ||= ProyeksiApp::Configuration.sentry_dsn.presence || ENV["SENTRY_DSN"].presence
        end
        ...
        ##
        # Build the sentry context from the proyeksiapp logging context
        def build_sentry_context(sentry_scope, log_context)
          ...
          extra = ::ProyeksiApp::Logging.extend_payload! log_context[:extra] || {}, { sentry_scope: sentry_scope }
