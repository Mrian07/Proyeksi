...
module ProyeksiApp
  ...
  module Database
    ...
    def self.migrations_pending?(ensure_fresh: false)
      cache_key = ProyeksiApp::Cache::CacheKey.key('database_migrations')
    ...
    def self.check!
      if !postgresql?
        message = "ProyeksiApp requires a PostgreSQL database, but database adapter is set to #{adapter_name}."

        if adapter_name.match?(/mysql/i)
          message << " As MySql used to be supported, there is a migration script to ease the transition " \
                     "(https://www.proyeksiapp.org/deprecating-mysql-support/)."
        end
        ...
        message = "The next major release of ProyeksiApp (v12) will require PostgreSQL 13 or later.\n" \
                  "You can anticipate this upgrade by updating your database installation by following the guide at " \
                  "https://docs.proyeksiapp.org/installation-and-operations/misc/migration-to-postgresql13/"
    ...
    # ProyeksiApp::Database.postgresql? returns true, if we have a postgresql DB
    # Also allows specification of a connection e.g.
    # ProyeksiApp::Database.postgresql?(my_connection)
