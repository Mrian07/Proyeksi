  ...
  module Storage
    class << self
      ...
      def mount_information
        return {} unless ProyeksiApp::Configuration.show_storage_information?

        list_mounts(known_storage_paths)
      end

      ##
      # List available storage paths of ProyeksiApp
      def known_storage_paths
        paths = {}

        # SCM vendors
        ProyeksiApp::SCM::Manager.managed_paths.each do |vendor, path|
          paths[vendor] = {
            path: path,
