...
require_dependency 'proyeksi_app/ui/extensible_tabs'
require_dependency 'config/constants/api_patch_registry'
require_dependency 'config/constants/proyeksi_app/activity'
...
module ProyeksiApp::Plugins
  module ActsAsOpEngine
    ...
    module InstanceMethods
      ...
      # <plugin root>/lib/proyeksiapp/<plugin name>/patches/<patched_class>_patch.rb
      # Make sure the patch module has the name the Rails autoloading expects.
      #
      # Example:
      #  patches [:IssuesController]
      # This looks for ProyeksiApp::XlsExport::Patches::IssuesControllerPatch
      #  in proyeksiapp/xls_export/patches/issues_controller_patch.rb
      ...
      #
      # Useful when adding a field to an ProyeksiApp core model. We discourage adding
      ...
      #
      # See PermittedParams in ProyeksiApp for available models
      #
      # Example:
      ...
      # Register a plugin with ProyeksiApp
      #
      ...
      # options:       An options Hash, at least :requires_proyeksiapp is recommended to
      #                define the minimal version of ProyeksiApp the plugin is compatible with
      ...
      def add_tab_entry(key, name:, partial:, path:, label:, only_if: nil)
        ::ProyeksiApp::Ui::ExtensibleTabs.add(key, name: name, partial: partial, path: path, label: label, only_if: only_if)
      end
      ...
      def activity_provider(event_type, options = {})
        ProyeksiApp::Activity.register(event_type, options)
      end
      ...
      def class_inflection_override(overrides)
        self.class.initializer "#{engine_name}.class_inflection_override" do
          ProyeksiApp::Inflector.inflection(overrides)
        end
      end
    end
  end
