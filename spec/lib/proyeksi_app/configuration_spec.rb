...
describe ProyeksiApp::Configuration do
  describe '.load_config_from_file' do
    ...
    before do
      ...
      ProyeksiApp::Configuration.load(file: 'configfilename')
    end

    it 'should merge the config from the file into the given config hash' do
      expect(ProyeksiApp::Configuration['somesetting']).to eq('foo')
      expect(ProyeksiApp::Configuration[:somesetting]).to eq('foo')
      expect(ProyeksiApp::Configuration.somesetting).to eq('foo')
    end
  end
  ...
  describe '.load_env_from_config' do
    describe 'with a default setting' do
      let(:config) do
        ProyeksiApp::Configuration.send(:load_env_from_config, {
  ...
  describe '.load_overrides_from_environment_variables' do
    ...
    before do
      stub_const('ProyeksiApp::Configuration::ENV_PREFIX', 'OPTEST')

      ProyeksiApp::Configuration.send :override_config!, config, env_vars
    end
  ...
  describe '.with' do
    before do
      expect(ProyeksiApp::Configuration).to receive(:load_config_from_file) do |_filename, _env, config|
        config.merge!('somesetting' => 'foo')
      end
      ProyeksiApp::Configuration.load(env: 'test')
    end

    it 'should return the overridden the setting within the block' do
      expect(ProyeksiApp::Configuration['somesetting']).to eq('foo')

      ProyeksiApp::Configuration.with 'somesetting' => 'bar' do
        expect(ProyeksiApp::Configuration['somesetting']).to eq('bar')
      end
  ...
  describe '.convert_old_email_settings' do
    ...
    context 'with delivery_method' do
      before do
        ProyeksiApp::Configuration.send(:convert_old_email_settings, settings,
  ...
  describe '.migrate_mailer_configuration!' do
    ...
    it 'does nothing if no legacy configuration given' do
      ProyeksiApp::Configuration['email_delivery_method'] = nil
      expect(Setting).to_not receive(:email_delivery_method=)
      expect(ProyeksiApp::Configuration.migrate_mailer_configuration!).to eq(true)
    end

    it 'does nothing if email_delivery_configuration forced to legacy' do
      ProyeksiApp::Configuration['email_delivery_configuration'] = 'legacy'
      expect(Setting).to_not receive(:email_delivery_method=)
      expect(ProyeksiApp::Configuration.migrate_mailer_configuration!).to eq(true)
    end

    it 'does nothing if setting already set' do
      ProyeksiApp::Configuration['email_delivery_method'] = :sendmail
    ...
    it 'migrates the existing configuration to the settings table' do
      ProyeksiApp::Configuration['email_delivery_method'] = :smtp
      ProyeksiApp::Configuration['smtp_password'] = 'p4ssw0rd'
      ProyeksiApp::Configuration['smtp_address'] = 'smtp.example.com'
      ProyeksiApp::Configuration['smtp_port'] = 587
      ProyeksiApp::Configuration['smtp_user_name'] = 'username'
      ProyeksiApp::Configuration['smtp_enable_starttls_auto'] = true
      ProyeksiApp::Configuration['smtp_ssl'] = true
  ...
  describe '.reload_mailer_configuration!' do
    ...
    it 'uses the legacy method to configure email settings' do
      ProyeksiApp::Configuration['email_delivery_configuration'] = 'legacy'
      expect(ProyeksiApp::Configuration).to receive(:configure_legacy_action_mailer)
  ...
  describe '.configure_legacy_action_mailer' do
    ...
    it 'should enable deliveries and configure ActionMailer smtp delivery' do
      ...
      expect(action_mailer).to receive(:smtp_settings=).with(address: 'smtp.example.net',
                                                             port: '25')
      ProyeksiApp::Configuration.send(:configure_legacy_action_mailer, config)
    end
  end
  ...
  describe '.configure_cache' do
    ...
    context 'with cache store already set' do
      ...
      context 'with additional cache store configuration' do
        before do
          ProyeksiApp::Configuration['rails_cache_store'] = 'bar'
        end

        it 'changes the cache store' do
          ProyeksiApp::Configuration.send(:configure_cache, application_config)
      ...
      context 'without additional cache store configuration' do
        before do
          ProyeksiApp::Configuration['rails_cache_store'] = nil
        end
  ...
  context 'helpers' do
    describe '#direct_uploads?' do
      let(:value) { ProyeksiApp::Configuration.direct_uploads? }
