    ...
    context 'User links' do
      ...
      context 'User link via mail' do
        ...
        context 'when visible user exists' do
          ...
          context 'with only_path false (default)', with_settings: { host_name: "proyeksiapp.org" } do
            ...
            it_behaves_like 'format_text produces' do
              ...
              let(:expected) do
                <<~EXPECTED
                  <p class="op-uc-p">
                    Link to <a class="user-mention op-uc-link" href="http://proyeksiapp.org/users/#{user.id}" title="User Foo Barrit">Foo Barrit</a>
                  </p>
                EXPECTED
              end
            end
