...
describe ProyeksiApp::Notifications do
  ...
  describe '.send' do
    before do
      ...
      ProyeksiApp::Notifications.subscribe('notifications_spec_send', &probe)
    ...
    it 'should deliver a notification' do
      ProyeksiApp::Notifications.send('notifications_spec_send', payload)
    end
  end
  ...
  describe '.subscribe' do
    it 'throws an error when no callback is given' do
      expect do
        ProyeksiApp::Notifications.subscribe('notifications_spec_send')
    ...
    describe 'clear_subscriptions:' do
      ...
      def example_with(clear:)
        ProyeksiApp::Notifications.subscribe(key) do |out|
          as << out
        end
        ProyeksiApp::Notifications.send(key, 1)

        ProyeksiApp::Notifications.subscribe(key, clear_subscriptions: clear) do |out|
          bs << out
        end
        ProyeksiApp::Notifications.send(key, 2)
      end
