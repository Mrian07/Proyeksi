...
describe ProyeksiApp::SCM::Adapters::Subversion do
  let(:root_url) { '/tmp/bar.svn' }
  let(:url) { "file://#{root_url}" }
  let(:config) { {} }
  let(:adapter) { ProyeksiApp::SCM::Adapters::Subversion.new url, root_url }
    ...
    describe '.check_availability!' do
      it 'should not be available' do
        ...
        expect { adapter.check_availability! }
          .to raise_error(ProyeksiApp::SCM::Exceptions::SCMUnavailable)
      end
      ...
      it 'should raise a meaningful error if shell output fails' do
        ...
        expect { adapter.check_availability! }
          .to raise_error(ProyeksiApp::SCM::Exceptions::SCMUnauthorized)
      end
    end
  end

  describe 'repository with authorization' do
    let(:adapter) { ProyeksiApp::SCM::Adapters::Subversion.new url, root_url, login, password }
  ...
  describe 'empty repository' do
      ...
      context 'with non-existing root_url' do
        ...
        it 'should fail' do
          expect { adapter.create_empty_svn }
            .to raise_error(ProyeksiApp::SCM::Exceptions::CommandFailed)
    ...
    describe '.check_availability!' do
      it 'should be marked empty' do
        ...
        expect { adapter.check_availability! }
          .to raise_error(ProyeksiApp::SCM::Exceptions::SCMEmpty)
      end
    end
  end
