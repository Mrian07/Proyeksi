def mock_global_permissions(permissions)
  ...
  allow(ProyeksiApp::AccessControl).to receive(:modules).and_wrap_original do |m, *args|
    m.call(*args) + mapped_modules.map { |name| { order: 0, name: name } }
  end
  allow(ProyeksiApp::AccessControl).to receive(:permissions).and_wrap_original do |m, *args|
    m.call(*args) + mapped
  end
  allow(ProyeksiApp::AccessControl).to receive(:global_permissions).and_wrap_original do |m, *args|
    m.call(*args) + mapped
  end
end

def mock_permissions(name, options = {})
  ::ProyeksiApp::AccessControl::Permission.new(
    name,
    { does_not: :matter },
