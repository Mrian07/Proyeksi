...
function toggleMemberFilter() {
  if (window.ProyeksiApp.guardedLocalStorage("showFilter") === "true") {
    window.ProyeksiApp.guardedLocalStorage("showFilter", 'false');
    hideFilter(filter);
    jQuery('#filter-member-button').removeClass('-active');
  }
  else {
    window.ProyeksiApp.guardedLocalStorage("showFilter", 'true');
    showFilter(filter);
