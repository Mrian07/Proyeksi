...
describe ProyeksiApp::Meeting::DefaultData, with_clean_fixture: true do
  let(:seeder) { BasicData::RoleSeeder.new }

  let(:roles) { [member, reader] }
  let(:member) { ProyeksiApp::Meeting::DefaultData.member_role }
  let(:reader) { ProyeksiApp::Meeting::DefaultData.reader_role }

  let(:member_permissions) { ProyeksiApp::Meeting::DefaultData.member_permissions }
  let(:reader_permissions) { ProyeksiApp::Meeting::DefaultData.reader_permissions }
