...
describe ProyeksiApp::GithubIntegration::HookHandler do
  describe '#process' do
    ...
    context 'with a supported event and a user' do
      ...
      before do
        allow(ProyeksiApp::Notifications).to receive(:send)
      end

      it 'sends a notification with the correct contents' do
        handler.process(hook, request, params, user)
        expect(ProyeksiApp::Notifications).to have_received(:send).with("github.pull_request", expected_params)
      end
